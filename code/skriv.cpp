...
GetBitmapFromCodepoint(program_memory *Memory, char Codepoint)
{
    ...
    {
        Memory->PlatformAPI.LoadGlyphToMemory(Font, Codepoint, Font->Name, Font->Size);
    }
...
internal void
RenderTextLine(bitmap *Buffer, program_memory *Memory, char *TextOut, v4 Color = {1, 1, 1, 1})
{
    r32 AtX = 0;
    r32 AtY = 0;
    while(*TextOut)
    {
        ...
        if(Glyph)
        {
            AtY = (r32)(Buffer->Height - Glyph->Height);
...
//
    ...
    RenderTextLine(Buffer, Memory, "A");
